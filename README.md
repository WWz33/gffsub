...
The same workflow can be written at top level with `gffsub <input.gff3> --qc`. QC writes a TSV table with `severity`, `code`, `line_idx`, `id`, and `message`. Current check codes are `invalid_gff_version`, `duplicate_id`, `invalid_sequence_region`, `invalid_coordinate`, `invalid_range`, `outside_sequence_region`, `invalid_strand`, `invalid_phase`, `invalid_cds_phase`, `missing_parent`, and `child_outside_parent`.
