...
Current checks report duplicate IDs, non-positive coordinates, invalid ranges, invalid strand or phase column values, invalid CDS phases, missing parents, and child features outside parent coordinates. Output is TSV so it can be filtered with standard command-line tools.
...
The same workflow can be written at top level with `gffsub <input.gff3> --qc`. QC writes a TSV table with `severity`, `code`, `line_idx`, `id`, and `message`. Current check codes are `duplicate_id`, `invalid_coordinate`, `invalid_range`, `invalid_strand`, `invalid_phase`, `invalid_cds_phase`, `missing_parent`, and `child_outside_parent`.
