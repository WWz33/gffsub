...
当前检查会报告非法 `##gff-version` directive、重复 ID、非法 `##sequence-region` directive、非正坐标、非法坐标范围、feature 超出 `##sequence-region` 边界、非法 strand 或 phase 列值、非法 CDS phase、Parent 缺失，以及 child feature 超出 parent 坐标范围。输出为 TSV，便于用常规命令行工具继续过滤。
...
同一工作流也可以写成顶层形式：`gffsub <input.gff3> --qc`。QC 输出 TSV 表，字段为 `severity`、`code`、`line_idx`、`id` 和 `message`。当前检查代码包括 `invalid_gff_version`、`duplicate_id`、`invalid_sequence_region`、`invalid_coordinate`、`invalid_range`、`outside_sequence_region`、`invalid_strand`、`invalid_phase`、`invalid_cds_phase`、`missing_parent` 和 `child_outside_parent`。
