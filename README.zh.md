...
当前检查会报告非法 `##gff-version` directive、非法列数、非法属性语法、重复属性 tag、非法 seqid、非法 feature type、重复 ID、非法或重复的 `##sequence-region` directive、非正坐标、非法坐标范围、非法 score 值、feature 超出 `##sequence-region` 边界、非法 strand 或 phase 列值、非法 CDS phase、Parent 缺失，以及 child feature 超出 parent 坐标范围。输出为 TSV，便于用常规命令行工具继续过滤。
